...
final class IconPack
{
    ...
    public function __construct( null|string|array $pack = null )
    {
        ...
        elseif ( \is_array( $pack ) ) {
            $this->name  = $pack['name'] ?? E_Value::error( 'Undefined icon pack name in provided array.' );
            $this->icons = $pack;
        }
        else {
            // TODO
    ...
    public function get(
        ...
        if ( ! $this->has( $icon ) ) {
            ...
            if ( $fallback && ! $this->has( $fallback ) ) {
                E_Value::error( 'Unable to resolve icon {icon}, no fallback {fallback} found.', ['icon' => $icon, 'fallback' => $fallback] );
                return EMPTY_STRING;
            }
            Log::warning(
